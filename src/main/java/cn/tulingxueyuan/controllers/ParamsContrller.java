...
public class ParamsContrller {
    ...
    public String params01(@RequestParam(value="username",defaultValue = "徐庶") String name){
        ...
        *       true 默认   必须要传入， 如果没有传入,或者传入不正确，则会报错:HTTP Status 400 - Required String parameter 'username' is not present
        *       false  可以不用必须传入值 ,如果没有传入会自动传入null--也就是前端可传可不传
        *       注意：不要用基础数据类型，因为基础数据类型无法接收null  。所以要用包装类型，包装类型
        *       defaultValue 默认值，当参数为null的时候会自动设置一个默认值
        *       注意：当设置了默认值，即设置了：defaultValue = "徐庶"， 可以省略required=false
        *
        */
        return "index.jsp";
    }
    ...
    /**
     * 处理请求参数乱码：
     *  *   GET:   直接设置tomcat目录下conf/server.xml  里面<Connector URIEncoding="UTF-8"  这种比较依赖服务器，如果你换了服务器，还得再新的服务器冲洗你设置
     *  *   POST:
     *  *   在servlet的时期： 在获取参数之前 设置 request.setCharacterEncoding("UTF-8")
     *  *                     但是在每个处理方法中都去设置一遍太麻烦
     *  *                     所以我们会用过滤器来处理编码的问题（终极解决方式）
